...
const NAV_ITEMS = [
  ...
  },
  {
    ...
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5 shrink-0" viewBox="0 0 256 256" fill="currentColor">
        <path d="M128,24A104,104,0,1,0,232,128,104.12,104.12,0,0,0,128,24Zm0,192a88,88,0,1,1,88-88A88.1,88.1,0,0,1,128,216Zm-8-80a8,8,0,0,1,8-8,20,20,0,1,0-20-20,8,8,0,0,1-16,0,36,36,0,1,1,44,35v1a8,8,0,0,1-16,0Zm16,52a12,12,0,1,1-12-12A12,12,0,0,1,136,188Z" />
      </svg>
    ),
  },
];
  ...
  return (
    ...
    >
      ...
      <div className={`flex items-center px-3 mb-6 ${collapsed ? 'flex-col gap-3' : 'justify-between'}`}>
        <Link href="/content" className="flex items-center gap-2.5 px-1.5" title={BRAND.name}>
          <span className="w-6 h-6 rounded-md bg-white text-black text-[11px] font-bold flex items-center justify-center shrink-0">
            {BRAND.logoLetter}
          </span>
          {!collapsed && <span className="text-[14px] font-semibold text-white">{BRAND.name}</span>}
        </Link>
        <button
