...
- If a `VERSION` file exists, read it. If it says `2.5.5` or higher, stop —
  already up to date. If it says `2.5.0`, `2.5.1`, `2.5.2`, `2.5.3` or `2.5.4`, apply ONLY the "Patch upgrades"
...
2. Create a branch: `git checkout -b upgrade/v2.5.5`.
...
1. Diff `git diff HEAD..v2.5.5 --stat` to see scope.
2. New files (the vast majority) can be checked out directly:
   `git checkout v2.5.5 -- <path>` for: `src/lib/crm/`, `src/lib/social/`,
...
5. Commit, merge the branch, deploy. Done — `VERSION` should read `2.5.5`
...
**From 2.5.4 → 2.5.5** — take the quality-ceiling adjustment directly:
...
**From 2.5.3 → 2.5.5** — take the hardened video-publishing files, then the 2.5.5 files above:
...
**From 2.5.2 → 2.5.5** — take the 2.5.3 files, then both patches above:
...
**From 2.5.1 → 2.5.5** — add the 2.5.2 studio file, then every patch above:

    git checkout v2.5.2 -- "src/app/social/page.tsx"
    git checkout v2.5.3 -- "src/lib/social/meta.ts" "src/lib/social/metrics.ts" "src/lib/social/publisher.ts" "src/app/api/social/tick/route.ts" SOCIAL.md VERSION CHANGELOG.md

**From 2.5.0 → 2.5.5** — apply the 2.5.1 files first, then every patch above:
...
**After upgrading to 2.5.5, check your Meta token scopes.** If Instagram
