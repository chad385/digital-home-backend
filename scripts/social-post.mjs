...
async function uploadFile(filePath) {
  ...
  const { signedUrl, path, publicUrl } = await api('POST', '/api/social/upload', {
  ...
  if (!res.ok) throw new Error(`Upload of ${filePath} failed: HTTP ${res.status}`);
  console.log(`  uploaded ${basename(filePath)} (${(size / 1024 / 1024).toFixed(1)} MB)`);
  return { path, publicUrl, kind: contentType.startsWith('video/') ? 'video' : 'image' };
}
